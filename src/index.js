...
function pressManageTask(folder, explorerContent) {
  explorerContent.elementOnDOM.removeChild(selectedElement);
  createTask(folder, explorerContent);
}
...
class Folder extends Element {
  constructor(elementType, elementClassName, elementId, parentElement) {
    ...
    this.taskNum = 0;
    // Add the event listener for double-clicking to open the explorer
  }
  ...
  addExplorer() {
    this.elementOnDOM.addEventListener("click", () => {
      ...
      // Ensure the closeIcon exists before adding the event listener
      if (this.closeIcon) {
...
function createTask(folder, explorerContent) {
  ...
  const taskInputPrompt = taskInputContainer.createChild(
    "span",
    "task-input-prompt",
    "task-input-prompt",
  );
  taskInputPrompt.elementOnDOM.innerText = "Please Enter Task info";
  const taskInputForm = taskInputContainer.createChild(
    "div",
    "task-input-form",
    "task-input-form",
  );
...
function displayTask(
  ...
  taskDisplayContainer.elementOnDOM.addEventListener("click", (e) => {
    selectedElement = taskDisplayContainer.elementOnDOM;
    const isSelected =
      taskDisplayContainer.elementOnDOM.getAttribute("data-selected") ===
      "true";
    if (isSelected) {
      taskInner.elementOnDOM.style.animation = "";
